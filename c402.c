...
**/
    ...
    tBTNodePtr pomoco = (*RootPtr);
    //pozice kam sa ma vkladat
    tBTNodePtr kam_vlozit;
    ...
    {
        while((nalezen == 0) && (pomoco)){
            kam_vlozit = pomoco;
            if(pomoco->Cont == Content)
            {
                nalezen = 420;
                // 420? vsak vime proc
            }
            //move to the right
                //krajni pravice
            else  if(pomoco->Cont < Content)
            {
                pomoco = pomoco->RPtr;
            }
            else if(pomoco->Cont > Content)
                //kupredu leva spatky ni krok
            {
                pomoco = pomoco->LPtr;
            }
    ...
    if(nalezen != 420)
    {
        tBTNodePtr pomoc = malloc(sizeof(struct tBTNode));
        if(pomoc != NULL)
        {
            //nahrani dat
            pomoc->Cont = Content;
        ...
        if(kam_vlozit == NULL)
        {
            ...
            //strom neni prazdny
        {
            if(kam_vlozit->Cont < Content)
            {
                // do prava
                kam_vlozit->RPtr = pomoc;
            }
            else
                // kupredu leva spatky ni krok
            {
                kam_vlozit->LPtr = pomoc;
            }
        }
    }
...
**/
    ...
    else
    {
        ...
        SInitP(&zasoba);
        tBTNodePtr pomahac = RootPtr;

        //najití nejlevějšího
        Leftmost_Preorder(RootPtr,&zasoba);


        // dokud není zásobník prázdný
        while (SEmptyP(&zasoba ) == FALSE)
        {
            //vrati hodnotu z vrcholu a současně provede pop
            pomahac = STopPopP(&zasoba);
            // vyhledání nejlevějšího
            Leftmost_Preorder(pomahac->RPtr, &zasoba);
...
**/
    ...
    else
    {
        ...
        tBTNodePtr pomahac = RootPtr;
        //najití nejlevějšího
        Leftmost_Inorder(RootPtr,&zasoba);
        ...
        while (SEmptyP(&zasoba ) == FALSE)
        {
            ...
            BTWorkOut(pomahac);
            // vyhledání nejlevějšího
            Leftmost_Inorder(pomahac->RPtr, &zasoba);
...
**/
    ...
    {
        tBTNodePtr sluha = RootPtr;

        int kupredu_leva = 0;
        // zasobnik na ukazatele
        tStackP POINT;
        // zasobnik urcujici zda se tudmi uz slo
        tStackB BOO;
        // inicializace zásobníku ukazatelů
        SInitB(&BOO);
        SInitP(&POINT);
        //najití nejlevějšího
        Leftmost_Postorder(sluha, &POINT, &BOO);


        // dokud není zásobník prázdný
        while (SEmptyP(&POINT) == FALSE)
        {
            //vrati hodnotu z vrcholu a současně provede pop
            sluha = STopPopP(&POINT);
            SPushP(&POINT, sluha);
            bool k = STopPopB(&BOO);
            ...
            {
                SPushB(&BOO, FALSE);
                Leftmost_Postorder(sluha->RPtr, &POINT, &BOO);
...
**/
    ...
    else
    {
        ...
        do{
            ...
            {
                if(SEmptyP(&POINT) == FALSE)
                {
                    //ulozim hodnotu
                    (*RootPtr) = STopPopP(&POINT);
                }
            }
        } while(((*RootPtr) != NULL) || (!SEmptyP(&POINT)));
